...
func (a *App) ScanWithProgress(ctx context.Context, opts ScanOptions, progress ScanProgressFn) (ScanResult, error) {
	...
	stepSummary := func(providerID string, nodes []graph.ResourceNode, edges []graph.RelationshipEdge) (typeCounts map[string]int, sampleLabel string, sampleTotal int, sampleItems []string) {
		...
		switch providerID {
		case "ec2":
			primaryType = "ec2:instance"
			sampleLabel = "instances"
		case "elbv2":
			primaryType = "elbv2:load-balancer"
			sampleLabel = "load balancers"
		case "ecs":
			primaryType = "ecs:service"
			sampleLabel = "services"
		case "iam":
			primaryType = "iam:role"
			sampleLabel = "roles"
		case "s3":
			primaryType = "s3:bucket"
			sampleLabel = "buckets"
		case "rds":
			primaryType = "rds:db-instance"
			sampleLabel = "dbs"
		case "lambda":
			primaryType = "lambda:function"
			sampleLabel = "functions"
		case "dynamodb":
			primaryType = "dynamodb:table"
			sampleLabel = "tables"
		case "sqs":
			primaryType = "sqs:queue"
			sampleLabel = "queues"
		case "sns":
			primaryType = "sns:topic"
			sampleLabel = "topics"
		case "kms":
			primaryType = "kms:key"
			sampleLabel = "keys"
